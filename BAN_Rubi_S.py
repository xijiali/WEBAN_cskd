...
def main():
    ...
    parser.add_argument('--name', default='BAN_Rubi_S', type=str, help='name of run')
    ...
    kld_criterion=KDLossRubi(args.temp)
...
class KDLossRubi(nn.Module):
    def __init__(self, temp_factor):
        super(KDLossRubi, self).__init__()
        self.temp_factor = temp_factor
        self.kl_div = nn.KLDivLoss(reduction="sum")

    def forward(self, input, target):
        weights=torch.sigmoid(input.detach())
        log_p = torch.log_softmax(input/self.temp_factor*weights, dim=1)
        q = torch.softmax(target/self.temp_factor, dim=1)
        loss = self.kl_div(log_p, q)*(self.temp_factor**2)/input.size(0)
        return loss
